			...
			if( !isChecked ) {
				$(r).each(function() {
					$.each(this, function(key, value) {
						...
						// 2024.11.21 포커싱 추가
						$("#"+key).siblings("._formCheck").find("span:eq(0)").attr('tabindex','0');
						$("#"+key).siblings("._formCheck").find("span:eq(0)").css('border','5px solid red');
						$("#"+key).siblings("._formCheck").find("span:eq(0)").focus();
					});
				});
			}
		},
...
var transferToSSO = function(toId) {
	var toUri = "/sso/sso_from.jsp";
	var ssoPop = window.open(toUri+"?pageReloadCount=1&toId="+toId, 'popSSO'+toId, '');
	ssoPop.focus();
}

var transferToSSOAbility = function(toId, rtnUrl) {
	var toUri = "/sso/sso_from_rtn_url.jsp";
	var ssoPop = window.open(toUri+"?pageReloadCount=1&toId="+toId+"&rtnUrl="+rtnUrl, 'popSSO'+toId, '');
	ssoPop.focus();
}

var transferToSSORtnurl = function(toId, rtnUrl) {
	var toUri = "/sso/sso_from_rtn_url.jsp";
	var ssoPop = window.open(toUri+"?pageReloadCount=1&toId="+toId+"&rtnUrl="+rtnUrl, 'popSSO'+toId, '');
	ssoPop.focus();
}

var transferToSSORotate = function() {
    var today = new Date();
    var sec = today.getSeconds()%2;
    if( sec == 0 ){
      transferToSSO(12);
    }else{
      transferToSSO(13);
